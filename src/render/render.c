                            ...
                            const PlayerState *player, int32_t index)
{
    if (index < 0) {
        ...
        uint32_t end = tree->leaves[leaf].first + tree->leaves[leaf].count;
        for (uint32_t i = tree->leaves[leaf].first; i < end; i++) {
            uint16_t seg_idx = tree->leaf_segs[i];
            const BSPSeg *seg = &tree->segs[seg_idx];
            const Linedef *line = &map->linedefs[seg->linedef];
            draw_seg(map, player, seg->v1, seg->v2, line);
        }
        return;
    }
    ...
    if (s >= 0.0f) {
        ...
        bsp_render_node(tree, map, player, node->back);
    } else {
        bsp_render_node(tree, map, player, node->back);
        bsp_render_node(tree, map, player, node->front);
    }
